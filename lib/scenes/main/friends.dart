...
final vrchatMobileFriendsProvider =
    FutureProvider.family<VRChatMobileFriendsData, bool>((ref, offline) async {
  ...
  try {
    do {
      ...
      List<VRChatFriends> users =
          await vrchatLoginSession.friends(offline: offline, offset: offset);
      userList.addAll(users);
      if (!offline) {
        for (VRChatFriends user in users) {
          futureList.add(getWorld(
  ...
  return VRChatMobileFriendsData(
      locationMap: locationMap, instanceMap: instanceMap, userList: userList);
});
...
class VRChatMobileFriends extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    AsyncValue<VRChatMobileFriendsData> data =
        ref.watch(vrchatMobileFriendsProvider(offline));
    ...
    return data.when(
      ...
      error: (e, trace) {
        ...
        return ScrollWidget(
          onRefresh: () =>
              ref.refresh(vrchatMobileFriendsProvider(offline).future),
      ...
      data: (data) => ScrollWidget(
        ...
        child: ExtractionFriend(
          id: offline
              ? GridModalConfigType.offlineFriends
              : GridModalConfigType.onlineFriends,
