        ...
        children: [
          for (String line in text.split('\n')) ...[
            ...() {
              ...
              String? text = () {
                match = RegExp(
                        r'^(Twitter|twitter|TWITTER)([:˸：\s]{0,3})([@＠\s]{0,3})([0-9０-９a-zA-Z_]{1,15})$')
                    .firstMatch(line);
                if (match != null)
                  return "https://twitter.com/${match!.group(match!.groupCount)}";
                match = RegExp(
                        r'^(Github|github|GITHUB)([:˸：\s]{1,3})([0-9０-９a-zA-Z_]{1,38})$')
                    .firstMatch(line);
                if (match != null)
                  return "https://github.com/${match!.group(match!.groupCount)}";
                ...
                if (match != null)
                  return "${match!.group(match!.groupCount)}"
                      ...
                      .replaceAll("․", ".");
                match = RegExp(
                        r'^(Discord|discord|DISCORD)([:˸：\s]{1,3})(.{1,16}[#＃][0-9０-９]{4})$')
                    .firstMatch(line);
                if (match != null)
                  return "${match!.group(match!.groupCount)}"
                      .replaceAll("＃", "#");
                        ...
                        ..onLongPress = () {
                          if (Uri.tryParse(text!) != null) {
                            showModalBottomSheetStatelessWidget(
                              context: context,
                              builder: () =>
                                  ShareUrlListTile(url: Uri.parse(text)),
                            );
                          }
                        }
                          ...
                          if (DateTime.now().millisecondsSinceEpoch -
                            ...
                            if (Uri.tryParse(text!) == null) {
                              ...
                              Widget? value = await openInBrowser(
                                url: Uri.parse(text),
                                forceExternal:
                                    accessibilityConfig.forceExternalBrowser,
                              );
                              if (value != null) {
                                Navigator.push(
                                  context,
